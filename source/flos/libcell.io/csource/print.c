...
cell_error cell_io_printf_s(cell_string fmt, ...) {
    ...
    if((err = cell_fmt_format_list((cell_slice_type *) & s, fmt, v)) != CELL_NULL)
        return err;

    if((err = cell_os_write_a(stdout, (cell_slice_type *) & s)) != CELL_NULL)
        return err;
...
cell_error cell_io_printf_p(const char *fmt, ...) {
    ...
    if((err = cell_fmt_format_list((cell_slice_type *) & s, cell_string_c(fmt), v)) != CELL_NULL)
        return err;
